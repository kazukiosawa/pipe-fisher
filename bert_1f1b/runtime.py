...
class StageRuntime:
    ...
    def run_training_loop_with_1f1b_flushes(self, n, optimizer, recompute_step):
        ...
        if self.epoch > 0 and self.rank == 0:
            print("Rank = ", self.rank, "Epoch = ", self.epoch, " update steps: ", (n/self.update_interval-1), ", Finish one epoch by 1F1B. Average time per update step: ", (time.time()-epoch_start_time)/(n/self.update_interval-1))
